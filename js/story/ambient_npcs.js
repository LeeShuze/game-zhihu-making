...
(() => {
  ...
  const PAUSE_POOLS = {
    ...
    after_floor28: {
      ...
      candidates: [
        ...
        { id: "sisi", scenes: "home" },
        { id: "boss", scenes: "home" },
        { id: "mom27", scenes: "visit27" },
        { id: "boy27", scenes: "visit27" },
      ],
    },
  };
  ...
  function resolveScenes(spec, unlockList) {
    let scenes = [];
  ...
  function rollForPause(pauseId, resumeNpc, unlockList) {
    ...
    excludeResumeIds(pool.candidates, resumeNpc).forEach((spec) => {
      ...
      resolveScenes(spec, unlockList).forEach((s) => byId.get(spec.id).push(s));
    });
    ...
    const picks = shuffle(expanded).slice(0, n);
    ...
    for (const p of picks) {
      ...
      const ranked = shuffle(p.scenes).sort(
        (a, b) => (usedScenes[a] || 0) - (usedScenes[b] || 0)
      );
      let scene = null;
      for (const s of ranked) {
        if ((usedScenes[s] || 0) >= Math.min(2, listFloorSpots(s).length || 2)) {
          continue;
        }
        scene = s;
        break;
